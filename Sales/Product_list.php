    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
      <div class="container">
        ...
        <div id="navbar" class="collapse navbar-collapse navbar-inverse">
          ...
          <li class="active"><a href="../Sales/index.php"><span class="glyphicon glyphicon-log-out" aria-hidden="true"></span> 銷貨</a></li>
          ...
          <?php
            ...
            for($i = 1; $i <= $select->rowCount(); $i++){
                ...
                if(substr($row->Product_Remarks,0,11) != "ItemDeleted"){
                    echo "
                        <tr>
                            <th><input type='checkbox' name='Product_id[]' value='".$row->Product_id."'>
                            <input type='hidden' name='rows[]' value='$rows'></th>
                            <th>".$row->Productcode."</th>
                            <th>".$row->Productname."</th>";
                if(trim($row->Productname2) == ""){
                  echo "<th>0</th>";
                }else{
                  echo "<th>".$row->Productname2."</th>";
                }            
                          
                if(trim($row->Product_Qty) == ""){
                  $row->Product_Qty = 0;
                }
                            echo"<th>".$row->Product_Qty."</th>
                            <th>".$row->Product_Remarks."</th>
                            <th><input type='number' name='orderQty[]' value='0' min='0' max='".$row->Product_Qty."'></th>
                        </tr>
                    ";
                    $rows++;
                }
